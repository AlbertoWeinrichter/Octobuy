...
from source.models.publication import (
    Publication,
...
class PublicationEditorAPI(MethodView):
    ...
    def post(self):
        try:
            ...
            response = {"message": "Publication created"}
            return response, 201
    ...
    def get(self):
        try:
            ...
            response = PublicationListSchema(
                publication_list=[
                    PublicationSchema(**p._asdict()) for p in publications
                ]
            ).dict()
            ...
            responseObject = {"message": "Some error occurred: {error}".format(error=e)}
            return make_response(jsonify(responseObject)), 500
...
publication_blueprint.add_url_rule(
    "/api/v1/publication", view_func=publication_view, methods=["POST"]
)

publication_blueprint.add_url_rule(
    "/api/v1/publication", view_func=publication_view, methods=["GET"]
)
